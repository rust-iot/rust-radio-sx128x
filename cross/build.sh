...
# Create target dir so permissions are okay
mkdir -p target/

# Run actual build
if [ "$1" == "x86_64-unknown-linux-gnu" ]; then
    $DOCKER_CMD $DOCKER_IMG /bin/bash -c "cargo build --target=$1 --release --features embedded-spi/hal-linux && chmod 755 -R /work/target"

elif [ "$1" == "armv7-unknown-linux-gnueabihf" ]; then
    $DOCKER_CMD $ARMHF_ARGS $DOCKER_IMG /bin/bash -c "cargo build --target=$1 --release --features embedded-spi/hal-linux && chmod 755 -R /work/target"

elif [ "$1" == "x86_64-apple-darwin" ]; then
    cargo build --target=$1 --release

elif [ "$1" == "i686-pc-windows-gnu" ]; then
    $DOCKER_CMD $DOCKER_IMG /bin/bash -c "cargo build --target=$1 --release && chmod 755 -R /work/target"

...
impl <Conn, ConnError, Output, Input, PinError, Delay> Cursed for Sx128x <Conn, ConnError, Output, Input, PinError, Delay> {}


impl<T, ConnError, Output, Input, PinError, Delay> Sx128x<T, ConnError, Output, Input, PinError, Delay>
where
    ...
    Delay: delay::DelayMs<u32>,
{
    ...
    pub fn ffi_status(&mut self) -> Result<sx1280::RadioStatus_t, Sx128xError<ConnError, PinError>> {
    ...
    pub fn ffi_get_firmware_version(&mut self) -> Result<u16, Sx128xError<ConnError, PinError>> {
        // Update rust object pointer to c object context
        let mut ctx = self.c.unwrap();
...
mod tests {
    ...
    type Radio = Sx128x<embedded_spi::mock::Spi, (), embedded_spi::mock::Pin, embedded_spi::mock::Pin, (), embedded_spi::mock::Delay>;
    ...
    fn test_ffi_passthrough() {
        ...
        m.expect(&[
            Mt::set_high(&sdn),
            Mt::spi_busy(&spi, PinState::High),
        ]);

        assert_eq!(0, Radio::set_reset(ptr, true));
        assert_eq!(1, Radio::get_busy(ptr));
   ...
   #[test]
    fn test_ffi_firmware_version() {
        ...
        m.expect(vectors::firmware_version(&spi, &sdn, &delay, 1102));
        let version = radio.firmware_version().unwrap();
        m.finalise();
        assert_eq!(version, 1102);
    }
}
